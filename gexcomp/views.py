...
def json_win_len_bio_analysis(request, run_id, n_wins, win_len, win_step):
    ...
    for i in range(len(top_wins)):
        ...
        # DETAILS
        tbl_top_wins_html += "<td class=\"text-gray-500\" scope=\"row\">"
        tbl_top_wins_html += "<a class=\"text-secondary lnk-win-info\" href='javascript:;' data-win-start='{0}' data-win-end='{1}' onclick=\"displayWinInfo({0}, {1})\">(click)</a>".format(win["win_start"], win["win_end"])
        tbl_top_wins_html += "</td>"

        tbl_top_wins_html += "</tr>"


    #get top whole profile DEGs generated by R
    top_whole_profile_degs = []
    # generate top wins html table
    tbl_top_whole_profile_degs = ""
    for i in range(len(top_whole_profile_degs)):
        deg = top_whole_profile_degs[i]
        deg_ensl_id = deg[0]
        deg_pval = deg[1]
        deg_adj_pval = deg[2]
        deg_log2_fc = deg[3]

        tbl_top_whole_profile_degs += "<tr>"

        # Nr.
        tbl_top_whole_profile_degs += "<td class =\"text-gray-900\" scope=\"row\">"
        tbl_top_whole_profile_degs += str(i + 1)
        tbl_top_whole_profile_degs += "</td>"

        # Ensemble Gene ID
        tbl_top_whole_profile_degs += "<td class =\"text-gray-500\" scope=\"row\">"
        tbl_top_whole_profile_degs += deg_ensl_id
        tbl_top_whole_profile_degs += "</td>"

        # P-val
        tbl_top_whole_profile_degs += "<td class =\"text-gray-500\" scope=\"row\">"
        tbl_top_whole_profile_degs += str(deg_pval)
        tbl_top_whole_profile_degs += "</td>"

        # Adj P-val
        tbl_top_whole_profile_degs += "<td class =\"text-gray-500\" scope=\"row\">"
        tbl_top_whole_profile_degs += str(deg_adj_pval)
        tbl_top_whole_profile_degs += "</td>"

        # log2FC
        tbl_top_whole_profile_degs += "<td class =\"text-gray-500\" scope=\"row\">"
        tbl_top_whole_profile_degs += str(deg_log2_fc)
        tbl_top_whole_profile_degs += "</td>"
