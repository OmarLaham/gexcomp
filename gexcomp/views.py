...
def json_win_selected_bio_analysis(request, run_id, win_start, win_end):
    ...
    dea_results_df_path = path.join(settings.RUNS_DIR, run_id,
                                    'deg_group_original_vs_prediction_win_{0}_to_{1}.csv'.format(1226, 1359))  # .format(win_start, win_end) #TODO: replace
    df_dea = pd.read_csv(dea_results_df_path, index_col=0)
